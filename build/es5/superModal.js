...
var superModal = function () {
    ...
    _createClass(superModal, [{
        ...
        value: function sheet() {
            ...
            var allStyles = document.querySelector(styleParams.modalClass + "-styles");
            if (allStyles) {
                for (var i = 0, l = allStyles.length; i < l; i++) {
        ...
        value: function cache(callback) {
            ...
            var modalClass = this.options.settings.modalClass;

            // Set modal class
            for (var prop in selectors) {
                var val = selectors[prop];
                selectors[prop] = modalClass + (val ? "__" : "") + val;
            }

            this.dom = {};

            for (var className in selectors) {
        ...
        key: "_parseOptions",
        value: function _parseOptions(obj) {
            ...
            for (var prop in obj) {
                ...
                if ((typeof propVal === "undefined" ? "undefined" : _typeof(propVal)) === 'object') {
                    ...
                    for (var prop1 in propVal) {
                        var prop1Val = propVal[prop1];

                        if ((typeof prop1Val === "undefined" ? "undefined" : _typeof(prop1Val)) === 'object') {

                            for (var prop2 in prop1Val) {
                                var prop2Val = prop1Val[prop2];

                                if ((typeof prop2Val === "undefined" ? "undefined" : _typeof(prop2Val)) === 'object') {

                                    for (var prop3 in prop2Val) {
                                        var prop3Val = prop2Val[prop3];
        ...
        key: "_closeConstr",
        value: function _closeConstr(that, fn) {
            return function () {
                var answer = void 0;
                if (fn) answer = fn();
        ...
        key: "_removeImages",
        value: function _removeImages(els) {
            var dom = this.dom;
            if (dom.image.remove) {
        ...
        key: "_setText",
        value: function _setText(title, content, accept, decline, woContent) {
            var dom = this.dom,
                opt = this.options;
            // Setting content
        ...
        key: "_setMode",
        value: function _setMode(callback) {
            ...
            setTimeout(function () {
                var c = parseInt(that.dom.modal.clientHeight, 10),
        ...
        key: "_modBody",
        value: function _modBody(except) {
            var method = void 0,
                dom = this.dom,
                modalClass = this.options.settings.modalClass;
        ...
        key: "_toImg",
        value: function _toImg(href, param) {
            ...
            if (param === "alert") modeClass = modalClass + "--image";else modeClass = modalClass + "--image__alert";
        ...
        key: "show",
        value: function show() {
            var mainClass = this.dom.main.classList,
                modalClass = this.options.settings.modalClass,
                startAnimation = function startAnimation() {
                return mainClass.add(modalClass + "__loaded");
            };
            this.isOpened = true;
        ...
        key: "bind",
        value: function bind(accept, decline) {
            var that = this,
                dom = that.dom,
        ...
        key: "alert",
        value: function alert(title, content, btn, fn) {
            var dom = this.dom,
        ...
        key: "confirm",
        value: function confirm(title, content, accept, decline, _confirm, cancel) {
            var dom = this.dom,
            ...
            dom.accept.classList.remove(modalClass + "__hidden");
            // Set the text
            this._setText(object ? object["content"]["title"] || data.content.title : title || data.content.title, object ? object["content"]["text"] || data.content.text : content || data.content.text, object ? object["buttons"]["accept"]["content"] || data.buttons.accept.content : accept || data.buttons.accept.content, object ? object["buttons"]["decline"]["content"] || data.buttons.decline.content : decline || data.buttons.decline.content);
            // Rebind buttons
            this.bind(object ? object["buttons"]["accept"]["fn"] || data.buttons.accept.fn : _confirm || data.buttons.accept.fn, object ? object["buttons"]["decline"]["fn"] || data.buttons.decline.fn : cancel || data.buttons.decline.fn);
            // Show modal
            this.show();
        ...
        key: "paste",
        value: function paste(html, title, accept, decline, acceptFn, declineFn) {
            ...
            this._setText(title || data.content.title, null, accept || data.buttons.accept.content, decline || data.buttons.decline.content, true);
